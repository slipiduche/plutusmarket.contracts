...
mkBuyValidator mp nfts r ctx = case r of
  ...
  where
    ...
    checkFee price = fromInteger (Ada.getLovelace (Ada.fromValue (valuePaidTo info (feeAddr mp)))) >= 45 % 1000 * fromInteger price

    checkSellerOut :: PubKeyHash -> Integer -> Integer -> Bool
    checkSellerOut seller royPrct price = fromInteger (Ada.getLovelace (Ada.fromValue (valuePaidTo info seller))) >= (1000 - 45 - royPrct) % 1000 * fromInteger price
